...
public class AdminCatalogService {
 private final CategoryRepository categories;private final ProductRepository products;private final EntityManager em;private final MediaProperties media;
 public AdminCatalogService(CategoryRepository categories,ProductRepository products,EntityManager em,MediaProperties media){this.categories=categories;this.products=products;this.em=em;this.media=media;}
 ...
 private ProductView view(Product p){
  var images=p.getImages().stream().map(this::imageView).toList();String primary=images.stream().filter(ImageView::primaryImage).findFirst().or(()->images.stream().findFirst()).map(ImageView::publicUrl).orElse("");
 ...
 public List<ImageView> images(String pid){entity(pid);return em.createQuery("select x from ProductImage x where x.product.id=:pid order by x.displayOrder,x.id",ProductImage.class).setParameter("pid",pid).getResultList().stream().map(this::imageView).toList();}
 private ProductImage imageEntity(String pid,String id){return em.createQuery("select x from ProductImage x where x.product.id=:pid and x.id=:id",ProductImage.class).setParameter("pid",pid).setParameter("id",id).getResultStream().findFirst().orElseThrow(this::missing);}
 ...
 @Transactional public ImageView saveImage(String pid,String id,ImageInput r){
  ...
  if(r.primaryImage())clearPrimary(pid);
  if(id==null){x.setId(id());x.setProduct(p);}x.setUrl(r.url());x.setAltText(r.altText());x.setPrimaryImage(r.primaryImage());x.setDisplayOrder(r.displayOrder());
